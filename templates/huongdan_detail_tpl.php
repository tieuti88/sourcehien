  ...
  <div class="info-title"><h2>Các Bài Viết Khác</h2></div>
  <div class="info-content">
  <div class="text">
  <?php if($tintuc_khac){?>
    <ul style=" list-style: url(images/icon.png) inside;">
      <?php for($i=0;$i<count($tintuc_khac); $i++){?>
      <li><a href="huong-dan/<?=$tintuc_khac[$i]['tenkhongdau']?>-<?=$tintuc_khac[$i]['id']?>.html">
        <?=$tintuc_khac[$i]['ten']?>
        (
    ...
    <?php }else{?><div style="color:#F00; padding: 10px; font-size: 13px;"><?=_tb?></div><?php } ?>
  </div>
  </div>
