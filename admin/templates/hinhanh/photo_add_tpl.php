...
<form name="frm" method="post" action="index.php?com=hinhanh&act=save_photo&idc=<?=$_REQUEST['idc']?>" enctype="multipart/form-data" class="nhaplieu">

<?php for($i=0; $i<5; $i++){?>
	
	<b>Hình ảnh <?=$i+1?></b> <input type="file" name="file<?=$i?>" /> <?=_hinhanh_type?><br />	       
	<b>Class <?=$i+1?></b> <input type="text" name="mota<?=$i?>" value="<?=@$item['mota<?=$i?>']?>" class="input" /><br /><br>	       
