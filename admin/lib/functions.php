		...
		if($totalRows>$mxR)
		{
			...
			{	
				if(($i>((int)(($curPg-1)/$mxP))* $mxP) && ($i<=((int)(($curPg-1)/$mxP+1))* $mxP))
				{
					...
					{
						$paging1 .= " <a href='".$url."&curPage=".$i."'  style='display: inline-block; color: #333; padding: 0px 5px; background: #F5F5FF; font-weight: bold; border: 1px solid #CCC;'>".$i."</a> ";	
					}
					$end=$i;	
				}
			}//tinh paging
			...
			#{
				...
				$paging .=" <a href='".$url."&curPage=".($curPg-1)."' style='display: inline-block; color: #333; padding: 0px 5px; background: #F5F5FF; font-weight: bold; border: 1px solid #CCC;' >&#8249;</a> "; //ve truoc
			#}
			$paging.=$paging1; 
			#if(((int)(($curPg-1)/$mxP+1)*$mxP) < $totalPages)  
			#{
				#$paging .=" <a href='".$url."&curPage=".($end+1)."' class=\"{$class_paging}\" >&#8250;</a> "; //ke
				$paging .=" <a href='".$url."&curPage=".($curPg+1)."' style='display: inline-block; color: #333; padding: 0px 5px; background: #F5F5FF; font-weight: bold; border: 1px solid #CCC;' >&#8250;</a> "; //ke
				
				$paging .=" <a href='".$url."&curPage=".($totalPages)."' style='display: inline-block; color: #333; padding: 0px 5px; background: #F5F5FF; font-weight: bold; border: 1px solid #CCC;' >&raquo;</a> "; //ve cuoi
			#}
		}
		$r3['curPage']=$curPg;
		$r3['source']=$r2;
...
function paging_home($r, $url='', $curPg=1, $mxR=5, $maxP=5, $class_paging='')
    {
        if($curPg<1) $curPg=1;
        if($mxR<1) $mxR=5;
        if($maxP<1) $maxP=5;
		...
		$paging .=" <a href='".$url."' style='display: inline-block; font-size: 20px; line-height: 11px;' >&laquo;</a> "; //ve dau
				
				#$paging .=" <a href='".$url."&curPage=".($start-1)."' class=\"{$class_paging}\" >&#8249;</a> "; //ve truoc
				$paging .=" <a href='".$url."p=".($curPg-1)."' style='display: inline-block; font-size: 20px; line-height: 11px; ' >&#8249;</a> "; //ve truoc
			#}
			$paging.=$paging1; 
			#if(((int)(($curPg-1)/$mxP+1)*$mxP) < $totalPages)  
			#{
				#$paging .=" <a href='".$url."&curPage=".($end+1)."' class=\"{$class_paging}\" >&#8250;</a> "; //ke
				$paging .=" <a href='".$url."p=".($curPg+1)."' style='display: inline-block; font-size: 20px; line-height: 11px;' >&#8250;</a> "; //ke
				
				$paging .=" <a href='".$url."p=".($totalPages)."' style='display: inline-block; font-size: 20px; line-height: 11px;' >&raquo;</a> "; //ve cuoi        
        }
        $r3['curPage']=$curPg;
        $r3['source']=$r2;
        $r3['paging']=$paging;
