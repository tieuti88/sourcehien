...
function save_photo(){
	...
	if($id){
			if($photo = upload_image("file", 'jpg|jpg|png|gif|JPG|jpeg|JPEG', _upload_hinhanh,$file_name)){
				$data['photo'] = $photo;
				$data['thumb'] = create_thumb($data['photo'], 190, 190, _upload_hinhanh,$file_name,2);
				$d->setTable('doitac');
				$d->setWhere('id', $id);
				$d->select();
				if($d->num_rows()>0){
			...
			redirect("index.php?com=doitac&act=man_photo");
			
	}else{ 			for($i=0; $i<5; $i++){
				if($data['photo'] = upload_image("file".$i, 'jpg|jpg|png|gif|JPG|jpeg|JPEG', _upload_hinhanh,$file_name.$i))
					{						
						$data['thumb'] = create_thumb($data['photo'], 190, 190, _upload_hinhanh,$file_name.$i,2);
