...
function get_items(){
	...
	{
	$id_up = $_REQUEST['hot'];
	$sql_spmoi = "SELECT id,tinnoibat FROM table_service where id='".$id_up."' ";
	$d->query($sql_spmoi);
	$row_spmoi= $d->result_array();
	$spmoi1=$row_spmoi[0]['tinnoibat'];
	if($spmoi1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_service SET tinnoibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_service SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	#-------------------------------------------------------------------------------
	$sql = "select * from #_service where id !=0";
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id, photo,thumb from #_service where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
...
function get_cats(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_spmoi = "SELECT id,hienthi FROM table_service_item where id='".$id_up."' ";
	$d->query($sql_spmoi);
	$row_spmoi= $d->result_array();
...
function get_list_cat($id=0){
	...
	$sql = "select * from #_service_cat order by stt asc";
	$d->query($sql);
	$cats = $d->result_array();
...
function get_list($id=0){
	global $d, $list;
	$sql = "select * from #_service_item order by id desc";
	$d->query($sql);
	$cats = $d->result_array();
