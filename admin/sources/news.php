...
function get_items(){
	...
	{
	$id_up = $_REQUEST['hot'];
	$sql_spmoi = "SELECT id,tinnoibat FROM table_news where id='".$id_up."' ";
	$d->query($sql_spmoi);
...
function save_item(){
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data)){			
			redirect("index.php?com=news&act=man&curPage=".$_REQUEST['curPage']."");
		}else
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id, photo,thumb from #_news where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
...
function get_cats(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_spmoi = "SELECT id,hienthi FROM table_news_item where id='".$id_up."' ";
	$d->query($sql_spmoi);
	$row_spmoi= $d->result_array();
...
function get_list_cat($id=0){
	...
	$sql = "select * from #_news_cat order by stt asc";
	$d->query($sql);
	$cats = $d->result_array();
...
function get_list($id=0){
	global $d, $list;
	$sql = "select * from #_news_item order by id desc";
	$d->query($sql);
	$cats = $d->result_array();
