...
switch($act){
	...
	case "man_cat":
		get_cats();
		$template = "huongdan/cats";
		break;
	case "add_cat":		
		$template = "huongdan/cat_add";
		break;
	case "edit_cat":		
		get_cat();
		$template = "huongdan/cat_add";
...
function get_items(){
	...
	{
	$id_up = $_REQUEST['hot'];
	$sql_spmoi = "SELECT id,tinnoibat FROM table_huongdan where id='".$id_up."' ";
	$d->query($sql_spmoi);
	$row_spmoi= $d->result_array();
	$spmoi1=$row_spmoi[0]['tinnoibat'];
	if($spmoi1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_huongdan SET tinnoibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_huongdan SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	#-------------------------------------------------------------------------------
	$sql = "select * from #_huongdan where id !=0";
...
function save_item(){
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data)){			
			redirect("index.php?com=huongdan&act=man&curPage=".$_REQUEST['curPage']."");
		}else
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id, photo,thumb from #_huongdan where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
		...
		if($d->query($sql)){
			redirect("index.php?com=huongdan&act=man".$id_catt."");
		}else
...
function get_cats(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_spmoi = "SELECT id,hienthi FROM table_huongdan_item where id='".$id_up."' ";
	$d->query($sql_spmoi);
	$row_spmoi= $d->result_array();
	$spmoi1=$row_spmoi[0]['hienthi'];
	if($spmoi1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_huongdan_item SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_huongdan_item SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql = "select * from #_huongdan_item";
	...
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=huongdan&act=man_cat";
	$maxR=20;
	$maxP=10;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function get_cat(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=huongdan&act=man_cat");
	
	$sql = "select * from #_huongdan_item where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=huongdan&act=man_cat");
	$item = $d->fetch_array();	
}
...
function save_cat(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=huongdan&act=man_cat");
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	if($id){
		...
		$d->setTable('huongdan_item');
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=huongdan&act=man_cat&curPage=".$_REQUEST['curPage']."");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=huongdan&act=man_cat");
	}else{
		...
		$d->setTable('huongdan_item');
		if($d->insert($data))
			redirect("index.php?com=huongdan&act=man_cat");
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=huongdan&act=man_cat");
	}
}
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();			
		$sql = "delete from #_huongdan_item where id='".$id."'";
		if($d->query($sql))
			redirect("index.php?com=huongdan&act=man_cat");
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=huongdan&act=man_cat");
	}else transfer("Không nhận được dữ liệu", "index.php?com=huongdan&act=man_cat");
}

function get_list_cat($id=0){
	global $d, $list_cat;
	
	$sql = "select * from #_huongdan_cat order by stt asc";
	$d->query($sql);
	$cats = $d->result_array();
...
function get_list($id=0){
	global $d, $list;
	$sql = "select * from #_huongdan_item order by id desc";
	$d->query($sql);
	$cats = $d->result_array();
